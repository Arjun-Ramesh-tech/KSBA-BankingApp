  ...
  MongoClient.connect(url, function (err, db) {
    ...
    dbo
      ...
      .toArray(function (err, result) {
        ...
        if (
          result.length > 0 &&
          result[0].password.cur == req.body.psw &&
          b == a
        ) {
...
app.all("/forgot_userid_otp", function (req, res) {
  MongoClient.connect(url, function (err, db) {
    ...
    dbo
      ...
      .find({
        $and: [
          { regMob: "+91" + req.body.mno },
          { "pan.number": req.body.pan },
      ...
      .toArray(function (err, result) {
        ...
        if (result.length > 0) {
          for (var i = 0; i < result[0].accounts.creditCard.length; i++) {
            if (
              result[0].accounts.creditCard[i].cardNo == req.body.card &&
              result[0].accounts.creditCard[i].cvv == req.body.cvv &&
              result[0].accounts.creditCard[i].expiryDate == exp
            ) {
              flag = 0;
              res.render("forgot_passwd_otp.pug", {
                Mobile: parseFloat(result[0].regMob),
              });
            }
          }
        }
      ...
      dbo
        ...
        .toArray(function (err, result) {
          ...
          if (
            ...
            var newvalues = {
              $set: { password: { old1: ol2, old2: cu, cur: req.body.psw } },
            };
            dbo
              .collection("customers")
            ...
            if (
              ...
              var newvalues = {
                $set: {
                  "password.old1": ol2,
                  "password.old2": cu,
                  "password.cur": req.body.psw1,
                },
              };
              dbo
                .collection("customers")
                .updateOne(myquery, newvalues, function (err, resul) {
          ...
          if (req.body.psw == result[0].password.cur) {
            ...
            var newvalues = {
              $set: {
                ...
                "address.kyc.file": kyc,
                "address.kyc.status": "pending",
              },
            };
            dbo
              .collection("customers")
              .updateOne(myquery, newvalues, function (err, resut) {
                res.redirect("/view_profile");
              });
          } else {
            res.render("Change_Address.pug", { name: req.session.name });
    ...
    MongoClient.connect(url, function (err, db) {
      ...
      dbo
        ...
        .toArray(function (err, result) {
          if (err) throw err;
          if (
            req.body.psw == result[0].password.cur &&
            req.body.cpan == result[0].pan.number
          ) {
            ...
            var newvalues = {
              $set: {
                "pan.number": pan,
                "pan.kyc.file": kyc,
                "pan.kyc.status": "pending",
              },
            };
            dbo
              .collection("customers")
              .updateOne(ob1, newvalues, function (err, resut) {
                res.redirect("/view_profile");
              });
          } else {
            res.render("Update_PAN.pug", { name: req.session.name });
...
app.all("/view_profile_mail", function (req, res) {
  if (req.session.cid) {
    res.render("Update_Mail_and_Mobile.pug", {
    ...
    MongoClient.connect(url, function (err, db) {
      ...
      dbo
        ...
        .toArray(function (err, result) {
          ...
          dbo
            .collection("customers")
            .update(ob2, newvalues, function (err, resut) {
              dbo
                .collection("customers")
                .find(ob1)
                .toArray(function (err, result) {
        ...
        .toArray(function (err, result) {
          for (i = 0; i < result[0].accounts.creditCard.length; i++) {
            if (result[0].accounts.creditCard[i].cardNo == req.body.card) {
              newvalues = {
                $push: { blockedAccounts: result[0].accounts.creditCard[i] },
              };
              newvalues1 = {
                $pull: {
                  "accounts.creditCard": {
                    cardNo: result[0].accounts.creditCard[i].cardNo,
                  },
                },
              };
              dbo
                .collection("customers")
                .update(ob1, newvalues, function (err, result) {
                  if (err) console.log(err);
                  dbo
                    .collection("customers")
                    .update(ob1, newvalues1, function (err, resut) {
                      if (err) console.log(err);
